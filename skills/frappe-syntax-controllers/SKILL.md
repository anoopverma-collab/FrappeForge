...
class SalesOrder(Document):
    ...
    def recalculate(self):
        ...
        self.total = sum(item.amount for item in self.items)
        self.save()  # Whitelisted methods DO NOT auto-save changes to self!
...
### 2. NEVER call frappe.db.commit() in controllers [BLOCKED v16+]

```python
# WRONG - breaks Frappe transaction management (BLOCKED in v16)
def validate(self):
    frappe.db.commit()  # Throws error in v16; causes partial updates in v14/v15
...
### 6. self.is_new() is False in on_update
`self.is_new()` returns `True` only in `before_insert`, `autoname`, and `validate` (on first save).

```python
# WRONG - is_new() is False here because doc is already in DB
def on_update(self):
    if self.is_new():
        self.send_welcome_email()

# CORRECT - use after_insert or self.flags.in_insert
def after_insert(self):
    self.send_welcome_email()

def on_update(self):
    if self.flags.in_insert:
        # Also works, but after_insert is cleaner
        pass
```
```
