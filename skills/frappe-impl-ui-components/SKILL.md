...
Frappe pages are auto-discovered from the `page/` directory — NO hook registration is needed.
(`page_js` is a different hook used to inject JS into existing pages, not to create new ones.)

### Step 1: Create the page via bench

```bash
bench --site mysite.local make-page my-custom-page myapp
```

This scaffolds the directory structure:

```
myapp/myapp/page/my_custom_page/
├── __init__.py
├── my_custom_page.json   # page metadata (title, roles, module)
└── my_custom_page.js     # page controller
```

### Step 2: Define the page controller

```javascript
// myapp/myapp/page/my_custom_page/my_custom_page.js
...
| Feature | v14 | v15 | v16 |
|---|---|---|---|
| Desk URL | `/app` | `/app` | **`/desk` (Rerouted)** |
| Bootstrap Version | v4 | **v5** | v5 |
| Dialog API | Stable | Stable | Stable |
| Scanner API | Supported | Supported | Supported |
| Realtime Rooms | Site-wide | Site-wide | Site-wide |
